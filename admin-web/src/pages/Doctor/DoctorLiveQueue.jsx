...
import { Bell, Play, SkipForward, CheckCircle2, Clock, FileText, Stethoscope } from 'lucide-react';
...
export default function DoctorLiveQueue({ clinicId }) {
  ...
  // Navigate to full patient medical file
  const handleOpenPatientFile = (patientId, e) => {
    if (e) e.stopPropagation();
    navigate(`/doctor/patients/${patientId}`);
  };

  // Navigate to specific appointment history
  ...
  // Start consultation, update status, and navigate to the consultation page
  const handleStartConsultation = (id, appointmentId, e) => {
    e.stopPropagation();
    setQueue(prev => prev.map(item => {
      if (item.id === id) {
        ...
        {queue.length === 0 ? (
          ...
          queue.map((item) => {
            ...
            return (
              <div 
                key={item.id} 
                onClick={(e) => patientId && handleOpenPatientFile(patientId, e)}
              ...
              >
                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 mt-0.5">Type: {appointmentType}</p>
                    </div>
                  </div>
                  <div>{statusBadge}</div>
                </div>
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    {/* Medical File Button (Now links to the full record) */}
                    {patientId && (
                      <button
                        onClick={(e) => handleOpenPatientFile(patientId, e)}
                        className="p-2 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100 transition"
                        title="Open Full Medical File"
                      >
                        <FileText size={16} />
                      </button>
                    )}
                    ...
                    {(item.status === 'waiting' || item.status === 'calling') && (
                      <button
                        onClick={(e) => handleStartConsultation(item.id, appointmentId, e)}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 transition shadow-sm border border-emerald-500"
                    ...
                    {item.status === 'in_progress' && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-600 text-white hover:bg-blue-700 transition"
