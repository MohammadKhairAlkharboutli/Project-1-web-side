...
import { Bell, FileText, LoaderCircle, Play, RefreshCw, SkipForward, Stethoscope, Users } from "lucide-react";
...
export default function DoctorLiveQueue() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    try {
      ...
      setQueue(
        Array.isArray(data)
          ? data.filter(
              (item) => Number(item.clinicId) === Number(assignedClinicId),
            )
          : [],
      );
      setLoadState("ready");
  ...
  async function callNext() {
    await run("call", () => doctorQueueApi.callNext(assignedClinic.id), "The next patient is now being called.");
  }

  async function startConsultation(item) {
    const started = await run("start", () => doctorQueueApi.startConsultation(item.id), "Consultation started.");
    if (started) navigate(`/doctor/consultation/${item.appointmentId}`);
  }

  async function skipPatient(item) {
    await run(`skip-${item.id}`, () => doctorQueueApi.skip(item.id), `${getPatientName(item)} was skipped.`);
  }

  if (loadState === "loading") return <section className="mx-auto flex w-full max-w-6xl items-center justify-center gap-2 rounded-lg border border-slate-200 bg-card p-14 text-sm font-medium text-muted-foreground shadow-surface"><LoaderCircle className="h-5 w-5 animate-spin" /> Loading queue…</section>;
  if (loadState === "error") return <section className="mx-auto w-full max-w-6xl rounded-lg border border-rose-100 bg-rose-50 p-7 text-rose-800"><h1 className="font-semibold">Queue unavailable</h1><p className="mt-2 text-sm">{error}</p><button type="button" onClick={loadQueue} className="mt-5 inline-flex items-center rounded-md bg-destructive px-4 py-2 text-sm font-semibold text-destructive-foreground">Try again</button></section>;

  return <div className="mx-auto w-full max-w-6xl space-y-6 pb-10"><header className="flex flex-col gap-4 rounded-3xl bg-gradient-to-br from-[#1e61dc] to-[#3b9df5] p-6 text-white shadow-lg shadow-blue-500/20 sm:flex-row sm:items-center sm:justify-between sm:p-8"><div><p className="text-xs font-bold uppercase tracking-wider text-blue-100">Live operations</p><h1 className="mt-2 type-hero-title">Today’s Patient Queue</h1><p className="mt-1 text-sm text-blue-100">Only checked-in appointments appear here.</p></div><Button className="bg-white text-blue-700 hover:bg-blue-50" onClick={loadQueue}><RefreshCw size={15} /> Refresh</Button></header>
    ...
    <section className="rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-4 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><h2 className="text-lg font-black text-slate-900">Waiting queue</h2><p className="mt-1 text-sm text-slate-500">Your assigned clinic: {assignedClinic?.name || "Clinic not available"}</p></div><button type="button" onClick={callNext} disabled={!canCallNext || Boolean(action)} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-3 text-sm font-bold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"><Bell size={17} /> {action === "call" ? "Calling…" : canCallNext ? "Call next patient" : activePatient ? "Patient already active" : "No patients waiting"}</button></div>{waitingPatients.length ? <div className="divide-y divide-slate-100">{waitingPatients.map((item) => <article key={item.id} className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div className="flex min-w-0 items-center gap-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-sm font-black text-slate-700">{item.currentPosition ?? "—"}</span><div className="min-w-0"><h3 className="truncate text-sm font-bold text-slate-900">{getPatientName(item)}</h3><p className="mt-0.5 text-xs text-slate-500">{item.appointment?.type || "Visit"} · estimated wait {item.expectedWaitingTimeMinutes ?? 0} min</p></div></div><div className="flex items-center gap-2"><button type="button" onClick={() => navigate(`/doctor/patients/${item.appointment?.patientId || item.appointment?.patient?.id}?appointmentId=${item.appointmentId || item.appointment?.id}`)} className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-blue-600 transition hover:bg-blue-100" title="Open medical file" aria-label={`Open ${getPatientName(item)} medical file`}><FileText size={17} /></button></div></article>)}</div> : <div className="px-6 py-14 text-center"><Users className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-700">No patients are waiting.</p><p className="mt-1 text-sm text-slate-500">Patients appear here after an administrator checks in today&apos;s confirmed appointment.</p></div>}</section></div>;
}

function Metric({ label, value, tone }) {
  return <div className="rounded-lg border border-slate-200 bg-card p-4 shadow-surface"><p className="text-xs font-medium text-muted-foreground">{label}</p><p className={`mt-2 text-3xl font-semibold ${tone}`}>{value}</p></div>;
}
