...
const referralSchema = z.object({ type: z.enum(["EXTERNAL", "FOLLOW_UP"]), reason: z.string().trim().min(3, "Referral reason is required."), toClinicId: z.string(), toDoctorId: z.string() }).superRefine((value, context) => {
  if (value.type === "EXTERNAL" && !value.toClinicId && !value.toDoctorId) context.addIssue({ code: "custom", path: ["toClinicId"], message: "Choose a clinic or doctor." });
});
...
export default function ConsultationPage() {
  ...
  useEffect(() => {
    Promise.all([clinicsApi.getClinics(), doctorsApi.getDoctors()]).then(([clinics, doctors]) => setDestinations({ clinics: Array.isArray(clinics) ? clinics : [], doctors: Array.isArray(doctors) ? doctors : [] })).catch(() => setDestinations({ clinics: [], doctors: [] }));
  }, []);
  ...
  async function submitReferral(values) {
    ...
    try {
      await referralsApi.createReferral({ patientId: Number(state.appointment.patientId), type: values.type, reason: values.reason.trim(), ...(values.toClinicId ? { toClinicId: Number(values.toClinicId) } : {}), ...(values.toDoctorId ? { toDoctorId: Number(values.toDoctorId) } : {}) });
      referralForm.reset();
