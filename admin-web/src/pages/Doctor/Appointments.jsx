...
function AppointmentFilters({ search, setSearch, status, setStatus, priority, setPriority, dateMode, setDateMode, fromDate, setFromDate, toDate, setToDate, onReset }) {
  return <div className="space-y-3"><Input className="max-w-sm" placeholder="Search patient name or phone..." value={search} onChange={(event) => setSearch(event.target.value)} /><div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap"><NativeSelect className="w-full sm:w-44" value={status} onChange={(event) => setStatus(event.target.value)}>{STATUS_OPTIONS.map((option) => <NativeSelectOption key={option.value} value={option.value}>{option.label}</NativeSelectOption>)}</NativeSelect><NativeSelect className="w-full sm:w-40" value={priority} onChange={(event) => setPriority(event.target.value)}><NativeSelectOption value="all">All priorities</NativeSelectOption><NativeSelectOption value="1">Normal priority</NativeSelectOption><NativeSelectOption value="2">High priority</NativeSelectOption></NativeSelect><NativeSelect className="w-full sm:w-40" value={dateMode} onChange={(event) => setDateMode(event.target.value)}>{DATE_MODE_OPTIONS.map((option) => <NativeSelectOption key={option.value} value={option.value}>{option.label}</NativeSelectOption>)}</NativeSelect>{dateMode === "custom" ? <><Input type="date" className="w-full sm:w-40" value={fromDate} onChange={(event) => setFromDate(event.target.value)} aria-label="Appointments from date" /><Input type="date" className="w-full sm:w-40" value={toDate} onChange={(event) => setToDate(event.target.value)} aria-label="Appointments to date" /></> : null}<Button variant="outline" onClick={onReset}>Reset filters</Button></div></div>;
}
