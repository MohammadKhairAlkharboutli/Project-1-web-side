...
export default function DoctorLeaves() {
  ...
  return <div className="space-y-8 pb-10">
    ...
    {loadState === "ready" ? <section className="overflow-hidden rounded-3xl border border-slate-200/60 bg-white shadow-xs"><div className="flex items-center justify-between border-b border-slate-100 p-6"><div><h2 className="text-sm font-bold text-slate-800">Your time-off entries</h2><p className="mt-1 text-xs text-slate-500">These are active schedule exceptions.</p></div><span className="rounded-xl bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">Total: {leaves.length}</span></div><div className="overflow-x-auto"><table className="w-full border-collapse text-left"><thead><tr className="border-b border-slate-100 bg-slate-50/70 text-[11px] font-bold uppercase tracking-wider text-slate-400"><th className="px-6 py-4">Date</th><th className="px-6 py-4">Type</th><th className="px-6 py-4">Hours</th><th className="px-6 py-4">Reason</th><th className="px-6 py-4 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">{leaves.length ? leaves.map((leave) => <tr key={leave.id} className="transition-all hover:bg-slate-50/50"><td className="flex items-center gap-2 px-6 py-4 font-bold text-slate-900"><Calendar size={15} className="text-[#1e61dc]" />{formatDate(leave.exceptionDate)}</td><td className="px-6 py-4"><span className={`rounded-xl px-2.5 py-1 text-[10px] font-bold ${leave.startTime ? "border border-amber-200/50 bg-amber-50 text-amber-700" : "border border-blue-200/50 bg-blue-50 text-[#1e61dc]"}`}>{leave.startTime ? "Partial hours" : "Full day"}</span></td><td className="px-6 py-4 text-slate-600">{leave.startTime ? <span className="flex items-center gap-1.5 font-semibold text-slate-700"><Clock size={14} className="text-slate-400" />{formatTime(leave.startTime)} – {formatTime(leave.endTime)}</span> : "Whole day"}</td><td className="max-w-xs truncate px-6 py-4 text-slate-600">{leave.reason || "No reason provided"}</td><td className="px-6 py-4 text-right"><button type="button" onClick={() => setDeleteTarget(leave)} className="rounded-xl p-2 text-rose-500 transition-all hover:bg-rose-50" title="Remove time off" aria-label={`Remove time off on ${formatDate(leave.exceptionDate)}`}><Trash2 size={16} /></button></td></tr>) : <tr><td colSpan="5" className="px-6 py-14 text-center text-sm text-slate-500">No time-off entries yet.</td></tr>}</tbody></table></div></section> : null}
