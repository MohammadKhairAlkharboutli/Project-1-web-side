...
function getLeaveDate(value) {
  const dateOnly = String(value || "").slice(0, 10);
  return dateOnly ? new Date(`${dateOnly}T00:00:00`) : null;
}

function getLeaveTiming(leave) {
  const date = getLeaveDate(leave.exceptionDate);
  if (!date || Number.isNaN(date.getTime())) return "past";
...
function formatDate(value) {
  const date = getLeaveDate(value);
  return !date || Number.isNaN(date.getTime()) ? String(value || "Not recorded").slice(0, 10) : new Intl.DateTimeFormat(undefined, { dateStyle: "medium" }).format(date);
}
...
function LeaveTable({ leaves, onDelete, text }) {
  if (!leaves.length) return <div className="px-6 py-14 text-center text-sm text-slate-500">{text("No time-off entries match this view.")}</div>;

  return (
    <div className="overflow-x-auto">
      <table className="w-full border-collapse text-left">
        <thead><tr className="border-b border-slate-100 bg-slate-50/70 text-[11px] font-bold uppercase tracking-wider text-slate-400"><th className="px-6 py-4">{text("Date")}</th><th className="px-6 py-4">{text("Timing")}</th><th className="px-6 py-4">{text("Type")}</th><th className="px-6 py-4">{text("Hours")}</th><th className="px-6 py-4">{text("Reason")}</th><th className="px-6 py-4 text-right">{text("Actions")}</th></tr></thead>
        <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
          {leaves.map((leave) => {
            const upcoming = getLeaveTiming(leave) === "upcoming";
            const partial = Boolean(leave.startTime);
            return <tr key={leave.id} className="transition-colors hover:bg-slate-50/50"><td className="px-6 py-4 font-bold text-slate-900"><span className="flex items-center gap-2"><Calendar size={15} className="text-[#1e61dc]" />{formatDate(leave.exceptionDate)}</span></td><td className="px-6 py-4"><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold ${upcoming ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-100 text-slate-600"}`}>{upcoming ? text("Upcoming") : text("Past")}</span></td><td className="px-6 py-4"><span className={`rounded-xl border px-2.5 py-1 text-[10px] font-bold ${partial ? "border-amber-200/50 bg-amber-50 text-amber-700" : "border-blue-200/50 bg-blue-50 text-[#1e61dc]"}`}>{text(partial ? "Partial hours" : "Full day")}</span></td><td className="px-6 py-4 text-slate-600">{partial ? <span className="flex items-center gap-1.5 font-semibold text-slate-700"><Clock size={14} className="text-slate-400" />{formatTime(leave.startTime)} – {formatTime(leave.endTime)}</span> : text("Whole day")}</td><td className="max-w-xs truncate px-6 py-4 text-slate-600">{leave.reason || text("No reason provided")}</td><td className="px-6 py-4 text-right"><button type="button" onClick={() => onDelete(leave)} className="rounded-xl p-2 text-rose-500 transition-colors hover:bg-rose-50" title={text("Remove time off")} aria-label={`${text("Remove time off")} ${formatDate(leave.exceptionDate)}`}><Trash2 size={16} /></button></td></tr>;
          })}
        </tbody>
      </table>
    </div>
...
export default function DoctorLeaves() {
  ...
  const sortedLeaves = useMemo(() => [...leaves].sort((first, second) => {
    const firstDate = getLeaveDate(first.exceptionDate)?.getTime() ?? 0;
    const secondDate = getLeaveDate(second.exceptionDate)?.getTime() ?? 0;
    const firstUpcoming = getLeaveTiming(first) === "upcoming";
    const secondUpcoming = getLeaveTiming(second) === "upcoming";
    if (firstUpcoming !== secondUpcoming) return firstUpcoming ? -1 : 1;
    return firstUpcoming ? firstDate - secondDate : secondDate - firstDate;
  }), [leaves]);
  const upcomingLeaveCount = useMemo(() => leaves.filter((leave) => getLeaveTiming(leave) === "upcoming").length, [leaves]);
  const visibleLeaves = useMemo(() => leaveView === "all" ? sortedLeaves : sortedLeaves.filter((leave) => getLeaveTiming(leave) === leaveView), [leaveView, sortedLeaves]);
  ...
  async function createLeave() {
    ...
    setNotice("");
    try {
      await doctorLeavesApi.createLeave({ exceptionDate: values.exceptionDate, ...(values.isPartial ? { startTime: `${values.startTime}:00`, endTime: `${values.endTime}:00` } : {}), ...(values.reason.trim() ? { reason: values.reason.trim() } : {}) });
      leaveForm.reset(defaultValues);
      setNotice("Time off saved. Any affected appointments have been updated and patients notified.");
      await loadLeaves();
    } catch (requestError) {
