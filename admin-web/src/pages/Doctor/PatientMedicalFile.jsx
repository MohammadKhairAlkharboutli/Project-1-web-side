...
import { Activity, AlertCircle, ArrowLeft, CalendarDays, FileText, HeartPulse, History, Mail, MapPin, Paperclip, Phone, ShieldAlert, UserRound } from "lucide-react";
...
export default function PatientMedicalFile() {
  ...
  return <section className="mx-auto max-w-7xl space-y-6 pb-10">
    <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-3"><Button variant="outline" size="icon-sm" onClick={() => navigate("/doctor/patients")} aria-label="Back to patients"><ArrowLeft className="h-4 w-4" /></Button><div><h1 className="text-2xl font-semibold tracking-tight text-slate-900">Patient Medical File</h1><p className="mt-1 text-sm text-slate-500">Live record from appointment #{state.appointment.id}</p></div></div>
      <Button variant="outline" size="sm" onClick={loadPatientFile}>Refresh record</Button>
    </div>
    ...
    <div className="grid gap-6 lg:grid-cols-[290px_minmax(0,1fr)]">
      ...
      <main className="min-w-0"><div className="mb-5 flex gap-2 overflow-x-auto rounded-xl border border-slate-200 bg-slate-50 p-1.5">{tabs.map((tab) => { const Icon = tab.icon; return <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`inline-flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium ${activeTab === tab.id ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}><Icon className="h-4 w-4" />{tab.label}</button>; })}</div>
        ...
        {currentTab?.id === "attachments" && <AttachmentsTab attachments={state.attachments} error={state.clinicalErrors.attachments} />}
...
function AttachmentsTab({ attachments, error }) { if (error) return <ClinicalAccessNotice error={error} />; const profileFiles = attachments.profileAttachments || []; const consultationFiles = attachments.historyAttachments || []; return profileFiles.length || consultationFiles.length ? <div className="space-y-5"><FileGroup title="Profile attachments" files={profileFiles} /><FileGroup title="Consultation attachments" files={consultationFiles} /></div> : <EmptyState title="No attachments" description="Profile and consultation files will appear here." />; }
function FileGroup({ title, files }) { return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-base font-semibold text-slate-900">{title}</h2>{files.length ? <div className="mt-4 space-y-3">{files.map((file) => <div key={file.id} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3"><FileText className="h-5 w-5 text-blue-600" /><div className="min-w-0"><p className="truncate text-sm font-medium text-slate-800">{file.originalName || "Attachment"}</p><p className="mt-0.5 text-xs text-slate-500">{file.fileType || "File"} · {formatDate(file.created_at || file.createdAt)}</p></div></div>)}</div> : <p className="mt-3 text-sm text-slate-500">No files in this group.</p>}</section>; }
