    ...
    <div className="min-h-screen w-full bg-[#f8fafc] p-6 font-sans text-slate-900 antialiased lg:p-8">
      <div className="mx-auto max-w-[1600px] space-y-6">
        ...
        <section className="space-y-6 rounded-[24px] border border-slate-100 bg-white p-6 shadow-xs sm:p-8"><div className="flex flex-col gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-base font-black text-slate-900 sm:text-lg">{activeDayLabel} {text("Schedule")}</h2><p className="mt-0.5 text-xs text-slate-400"><span className="font-bold text-slate-700">{assignedClinicName}</span></p></div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-sky-100 bg-sky-50 px-3.5 py-1.5 text-xs font-bold text-sky-600">{activeDaySlots.length} {text("Active Slots")}</span><Button onClick={openScheduleModal} disabled={!assignedClinicId} className="gap-2 rounded-xl bg-blue-600 text-white hover:bg-blue-700"><CalendarClock size={16} /><span>{hasExistingScheduleForDay ? `Request change for ${activeDayLabel}` : `Set ${activeDayLabel} availability`}</span></Button></div></div>
          {activeDaySlots.length ? <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">{activeDaySlots.map((slot, index) => <article key={slot.id || index} className="group relative space-y-3 overflow-hidden rounded-2xl border border-slate-200/90 bg-gradient-to-br from-slate-50/90 via-white to-sky-50/30 p-5 transition-all duration-300 hover:border-sky-400 hover:shadow-lg hover:shadow-sky-500/5"><div className="absolute top-0 right-0 left-0 h-1 bg-gradient-to-br from-[#1e61dc] to-[#3b9df5] opacity-60 transition-opacity group-hover:opacity-100" /><div className="flex flex-col items-start gap-2 pt-1"><div className="flex items-center gap-2.5"><div className="flex h-8 w-8 items-center justify-center rounded-xl bg-sky-100/70 text-[#1e61dc] shadow-2xs transition-colors group-hover:bg-[#1e61dc] group-hover:text-white"><Clock size={16} /></div><span className="text-sm font-extrabold tracking-tight text-slate-900">{formatDisplayTime(slot.startTime)} – {formatDisplayTime(slot.endTime)}</span></div><span className="rounded-lg border border-sky-200/50 bg-sky-100/60 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-sky-700">{slot.type}</span></div>{slot.notes ? <p className="border-l-2 border-sky-200 pl-10 text-xs font-medium text-slate-600">{slot.notes}</p> : <p className="pl-10 text-xs italic text-slate-400">No additional notes provided.</p>}</article>)}</div> : <div className="space-y-2 rounded-2xl border border-dashed border-slate-200 bg-slate-50/50 py-16 text-center"><p className="text-sm font-bold text-slate-700">No shifts scheduled for {activeDayLabel}</p><p className="text-xs text-slate-400">Use the {activeDayLabel} availability button above to set this day&apos;s hours.</p></div>}
        </section>
