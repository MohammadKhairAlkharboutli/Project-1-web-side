...
function ScheduleModal({ activeDayLabel, clinicName, isChangeRequest, scheduleForm, fields, append, remove, submitError, onClose, onSubmit }) {
  return <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm"><div className="w-full max-w-3xl overflow-hidden rounded-[32px] bg-white shadow-2xl"><div className="border-b border-slate-100 bg-[#f8fafc] p-8"><div className="flex items-center justify-between"><div className="flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-600/20"><CalendarClock size={24} /></div><div><h2 className="text-xl font-black text-slate-900">{isChangeRequest ? "Request Schedule Change" : "Set Availability"}</h2><p className="mt-0.5 text-xs font-bold text-slate-500">{activeDayLabel} · {clinicName}</p></div></div><button type="button" onClick={onClose} className="rounded-xl p-2.5 text-slate-400 transition-colors hover:bg-slate-200/50" aria-label="Close schedule form"><Plus className="rotate-45" size={24} /></button></div></div>
    <form onSubmit={onSubmit} className="space-y-6 p-8"><div className="flex items-start gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-xs font-bold leading-relaxed text-blue-700"><Info size={20} className="shrink-0 text-blue-500" /><p>{isChangeRequest ? "Your current schedule stays active until an administrator reviews this change request." : "Your first availability for this day will be saved immediately."}</p></div>{submitError ? <div className="flex items-center gap-3 rounded-2xl border border-rose-100 bg-rose-50 p-4 text-sm font-bold text-rose-600"><AlertCircle size={20} /><span>{submitError}</span></div> : null}
      <div className="max-h-[40vh] space-y-4 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-200">{fields.map((field, index) => <div key={field.id} className="group relative rounded-2xl border border-slate-200 bg-slate-50/50 p-5"><div className="grid grid-cols-1 gap-4 md:grid-cols-3"><div className="space-y-1.5"><label className="ml-1 text-[10px] font-black uppercase text-slate-400">Times</label><div className="flex items-center gap-2"><Input type="time" {...scheduleForm.register(`slots.${index}.startTime`)} className="h-10 rounded-xl bg-white text-xs font-bold" /><span className="text-slate-400">-</span><Input type="time" {...scheduleForm.register(`slots.${index}.endTime`)} className="h-10 rounded-xl bg-white text-xs font-bold" /></div>{scheduleForm.formState.errors.slots?.[index]?.startTime ? <p className="text-xs text-rose-600">{scheduleForm.formState.errors.slots[index].startTime.message}</p> : null}{scheduleForm.formState.errors.slots?.[index]?.endTime ? <p className="text-xs text-rose-600">{scheduleForm.formState.errors.slots[index].endTime.message}</p> : null}</div><div className="space-y-1.5"><label className="ml-1 text-[10px] font-black uppercase text-slate-400">Shift type</label><select {...scheduleForm.register(`slots.${index}.type`)} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-blue-500">{SCHEDULE_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</select></div><div className="space-y-1.5"><label className="ml-1 text-[10px] font-black uppercase text-slate-400">Notes</label><div className="flex gap-2"><Input placeholder="Optional notes" {...scheduleForm.register(`slots.${index}.notes`)} className="h-10 flex-1 rounded-xl bg-white text-xs font-bold" /><Button type="button" variant="ghost" disabled={fields.length === 1} onClick={() => remove(index)} className="h-10 w-10 rounded-xl border border-slate-100 p-0 text-rose-500 hover:bg-rose-50 disabled:opacity-40"><Trash2 size={16} /></Button></div>{scheduleForm.formState.errors.slots?.[index]?.notes ? <p className="text-xs text-rose-600">{scheduleForm.formState.errors.slots[index].notes.message}</p> : null}</div></div></div>)}</div>{scheduleForm.formState.errors.slots?.message ? <p className="text-sm text-rose-600">{scheduleForm.formState.errors.slots.message}</p> : null}
